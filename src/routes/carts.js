...
router.post('/', async (req, res) => {
  try {
    ...
    await fs.writeFile('./src/data/carts.json', JSON.stringify(carts, null, 2), 'utf-8'); // Actualizamos el archivo carrito.json
...
router.post('/:cid/product/:pid', async (req, res) => {
  try {
    ...
    const productIndex = carts[cartIndex].products.findIndex(p => p.product === productId); // Buscamos el índice del producto en el carrito
    if (cartIndex !== -1) {
      if (productIndex !== -1) {
